import React from "react";

export default function Artist() {
  return (
    <section className="bg-white">
      {/* ===== Artist 헤더 ===== */}
      <header className="max-w-6xl mx-auto px-6 pt-16 pb-8">
        <h1 className="text-4xl md:text-5xl font-serif font-bold tracking-tight text-neutral-900">
          Artist
        </h1>

        <nav className="mt-3 flex items-center gap-4 text-sm md:text-base text-neutral-600">
          <a href="#projects" className="font-medium hover:text-neutral-900">
            Projects With
          </a>
        </nav>
      </header>

      <div className="max-w-6xl mx-auto px-6 pb-16 md:pb-20">
        {/* ===== Projects With ===== */}
        <section id="projects">
          <h2 className="text-3xl md:text-4xl font-serif font-bold mb-16 text-center text-neutral-900">
            Projects With
          </h2>

          {/* ===== Art Fairs ===== */}
          <section className="mb-20">
            <h3 className="text-xl font-semibold mb-8 border-b pb-3 text-neutral-900">
              Art Fairs
            </h3>

            <div className="space-y-10">
              <div>
                <h4 className="text-xl font-semibold mb-2 text-[#6E4B9E]">
                  2026.04 Bank Art Fair
                </h4>

                <p className="text-neutral-700 leading-relaxed">
                  강채화, 권영심, 김희선, 남주희, 롱혼, 리미지미,
                  박가람, 박은정, 선, 선시연, 신승혜,
                  오지우, 임성자, 조유진
                </p>
              </div>
            </div>
          </section>

          {/* ===== Group Exhibitions ===== */}
          <section>
            <h3 className="text-xl font-semibold mb-8 border-b pb-3 text-neutral-900">
              Group Exhibitions
            </h3>

            <div className="space-y-10">

              <div>
                <h4 className="text-xl font-semibold mb-2 text-[#6E4B9E]">
                  2025.05 Group Exhibition
                </h4>

                <p className="text-neutral-700 leading-relaxed">
                  이상미, 채현정, 강채화, 윤지영, 최정희,
                  이현정, 박가람, 박지원, 구지희
                </p>
              </div>

              <div>
                <h4 className="text-xl font-semibold mb-2 text-[#6E4B9E]">
                  2025.06 Group Exhibition
                </h4>

                <p className="text-neutral-700 leading-relaxed">
                  백혜숙, 박예반, 최금곤, 윤혜성, 권미선,
                  신승혜, 양현숙, 박주현, 이해민
                </p>
              </div>

              <div>
                <h4 className="text-xl font-semibold mb-2 text-[#6E4B9E]">
                  2025.07 Group Exhibition
                </h4>

                <p className="text-neutral-700 leading-relaxed">
                  이지성, 권영심, 김성우, 이수향, 천미진,
                  정다솜, 정현준, 서한나, 구지희
                </p>
              </div>

              <div>
                <h4 className="text-xl font-semibold mb-2 text-[#6E4B9E]">
                  2025.08 Group Exhibition
                </h4>

                <p className="text-neutral-700 leading-relaxed">
                  김섭인, 남주희, 김이엘, 박은정,
                  이미애, 김지향, 신윤지,
                  김상현, 이유림
                </p>
              </div>

              <div>
                <h4 className="text-xl font-semibold mb-2 text-[#6E4B9E]">
                  2025.09 Group Exhibition
                </h4>

                <p className="text-neutral-700 leading-relaxed">
                  이하, 장도경, 이승비, 이채목,
                  오현정, 김희정, 오정숙,
                  박재영, 김윤기
                </p>
              </div>

              <div>
                <h4 className="text-xl font-semibold mb-2 text-[#6E4B9E]">
                  2025.10 Group Exhibition
                </h4>

                <p className="text-neutral-700 leading-relaxed">
                  박정현, 조유진, 최희주, 윤지선,
                  김은진, 김란수, 타이미,
                  이수연, 안형준
                </p>
              </div>

              <div>
                <h4 className="text-xl font-semibold mb-2 text-[#6E4B9E]">
                  2025.11 Group Exhibition
                </h4>

                <p className="text-neutral-700 leading-relaxed">
                  오지우, 백희경, 고희진, 양경혜,
                  김정은, 문혜영, 함효린,
                  김은재, 김경원
                </p>
              </div>

              <div>
                <h4 className="text-xl font-semibold mb-2 text-[#6E4B9E]">
                  2025.12 Group Exhibition
                </h4>

                <p className="text-neutral-700 leading-relaxed">
                  FlaYoon, 김경희, 조수연, 김잔디,
                  이하루, 이팝, 김지현,
                  오그림, 구지희
                </p>
              </div>

              <div>
                <h4 className="text-xl font-semibold mb-2 text-[#6E4B9E]">
                  2026.01 Group Exhibition
                </h4>

                <p className="text-neutral-700 leading-relaxed">
                  Moonjar, 김보연, 김민, 권예주,
                  박가람, 안세희, 윤그림,
                  정지은, 릴로
                </p>
              </div>

              <div>
                <h4 className="text-xl font-semibold mb-2 text-[#6E4B9E]">
                  2026.02 Group Exhibition
                </h4>

                <p className="text-neutral-700 leading-relaxed">
                  윤교, 전창석, 김희찬, 한혜선,
                  선, 윤은이, 신해달,
                  이서진, 남보라
                </p>
              </div>

              <div>
                <h4 className="text-xl font-semibold mb-2 text-[#6E4B9E]">
                  2026.03 Group Exhibition
                </h4>

                <p className="text-neutral-700 leading-relaxed">
                  박가람, 김지향, 박은정, 오현정,
                  신승혜, 강채화, 박지원,
                  롱혼, 윤주휘
                </p>
              </div>

              <div>
                <h4 className="text-xl font-semibold mb-2 text-[#6E4B9E]">
                  2026.04 Group Exhibition
                </h4>

                <p className="text-neutral-700 leading-relaxed">
                  문수정, 류형원, 김희선, 안성인,
                  훈어, 구지희, 비비테,
                  DANI, 오지우
                </p>
              </div>

              <div>
                <h4 className="text-xl font-semibold mb-2 text-[#6E4B9E]">
                  2026.05 Group Exhibition
                </h4>

                <p className="text-neutral-700 leading-relaxed">
                  윤선희, 전유리, Saera Hwang,
                  정경화, 정유진, 김선정,
                  김수란, 강지연, 최현주
                </p>
              </div>

            </div>
          </section>
        </section>
      </div>
    </section>
  );
}